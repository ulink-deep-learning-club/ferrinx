...
> **设计目标**: 为个人和小团队提供一个简单、灵活的 ONNX 模型推理服务。无需复杂的基础设施，一个二进制文件就能运行。
>
> **核心理念**: 专注于简单性和灵活性，同时保持合理的性能。
...
1. **简单性优先**: 代码应该易于理解和修改。没有复杂的抽象，除非真的需要。
2. **灵活性**: 组件之间松耦合。不想用 Redis？没问题。更喜欢 SQLite 而不是 PostgreSQL？完全可以。
3. **合理的性能**: 不追求极致速度，但也不慢。对于大多数用例来说已经足够好了。
4. **可扩展性**: 想添加功能？代码的组织方式让这变得简单直接。

### 1. Tensor 数据格式

**设计原则**: 使用显式 Tensor 格式，清晰明了，不容易出错。
...
- **简单模式（无 Redis）**：
  - 直接运行，无需额外依赖
  - 适合：开发测试、快速体验、学习 Rust
  - 一台机器就能跑起来

- **分布式模式（有 Redis）**：
  - 添加 Redis 后可以分布式运行
  - 适合：想试试分布式架构，或者确实有多个机器
  - 可选功能，不用也能玩得转
...
> 注意：这是玩具项目的性能预期，不是企业级 SLA。

| 指标 | 预期值 | 说明 |
|------|--------|------|
| 同步推理延迟 | 几十到几百毫秒 | 取决于模型大小 |
| 模型加载时间 | 几秒到十几秒 | 首次加载 |
| 并发推理数 | 4-8 | 可配置，看机器性能 |
| 模型缓存 | 基本有效 | 减少重复加载 |
| API 响应 | 通常 < 100ms | 不含推理时间 |

### 适用场景

- **个人项目**: 跑跑自己的模型，做点有趣的东西
- **学习实验**: 想学习 Rust + ONNX + Web 开发
- **快速原型**: 验证想法，不需要从头造轮子
- **小团队**: 内部工具，负载不大
...
> 虽然 Ferrinx 实现了基本的安全措施，但请记住这是一个**玩具项目**。如果你要处理敏感数据或部署到公网，请：
> - 进行额外的安全审查
> - 考虑使用成熟的企业级解决方案
> - 不要在生产环境中存储敏感数据而不做额外加固
...
2. **密码安全**
   - bcrypt 哈希（使用 bcrypt::DEFAULT_COST = 12）
   - 自动加盐存储
   - Bootstrap 自动生成安全随机密码
...
5. **内存安全**
   - 所有 `unsafe` 张量操作都带有 `SAFETY` 注释
   - 详细说明前提条件和不变量
   - 边界检查和类型验证

**SAFETY 注释示例**：
```rust
/// SAFETY: 输入张量数据必须满足以下条件：
/// 1. 数据指针必须非空且已对齐
/// 2. 数据长度必须等于 shape 各维度的乘积
/// 3. 数据生命周期必须长于张量使用期
unsafe fn create_tensor_from_raw(data: *const f32, len: usize, shape: &[usize]) -> Tensor {
    // ...
}
```
